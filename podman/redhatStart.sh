...
#     --network jThompsonSiteNet \
#     --env-file=.env \
#     --add-host $DBNAME:127.0.0.1 \
#     -p 8080:80 \
#     -d wordpress:latest

# podman exec --log-level debug  -i $WPNAME install_wordpress && apache2-foreground
...
podman run \
    ...
    -u node \
    -w /home/node/app \
    -v ./frontend-graphql:/home/node/app \
    -d node

# podman exec -it --log-level debug $FENAME yarn && yarn start