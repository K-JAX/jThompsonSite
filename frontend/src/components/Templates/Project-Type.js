...
class ProjectType extends Component {
	...
	executeProjectQuery = async () => {
		...
		const type = result.data.projectTypes.edges[0].node.slug;
		const title = result.data.projectTypes.edges[0].node.name;
			...
			<ProjectTypeTemplate>
				...
				<ProjectListings>
					{projects.map((project, index) => (
						<li
							className={(index + 1) % 2 ? "left" : "right"}
							key={project.id}
						>
								...
								<FigureLink
									...
									img={
										project.node.featuredImage !== null
											? project.node.featuredImage.sourceUrl
											: ""
									}
								/>
							</Link>
